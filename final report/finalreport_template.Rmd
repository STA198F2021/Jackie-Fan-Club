...
```{r health-insurance-sexuality, echo = FALSE}
data %>% 
...
```{r health-provider-sexuality, echo = FALSE}
data %>%
...
```{r health-insurance-gender, echo = FALSE}
data %>%
...
```{r health-provider-gender, echo = FALSE}
data %>%
  filter(is.na(healthProvideAcc) == FALSE) %>% 
ggplot(aes(x = gender, fill = healthProvideAcc)) +
  geom_bar(position = "fill") +
  labs (x = "Gender",
        y = "Frequency",
        fill = "Healthcare Provider Access",
